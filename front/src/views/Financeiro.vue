...
import { addCobranca } from '../api/cobranca.js';
import { addLancamento } from '../api/lancamento.js';
...
import lista_lancamentos from '../components/financeiro/lista_lancamentos.vue';
...
function cbAddLancamento(data, error) {
  if (data) {
    status.value = 'b';
    clearLancamento();
    financeiroVueStore.ativarRecarregar('lancamento');
  }
  if (error) {
